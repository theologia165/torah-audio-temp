...
def sid(s):
    nums=re.findall(r'\d+',s or ''); return 'H'+nums[-1] if nums else ''
def lemma(s):
...
def decode(m):
    ...
    prefixes=[]
    for p in parts[:-1]:
        if p.startswith('C'): prefixes.append('接続詞')
        elif p.startswith('R'): prefixes.append('前置詞')
        elif p.startswith('T') and 'd' in p: prefixes.append('定冠詞')
    core=parts[-1] if parts else ''
    if core=='Np': core_pos='固有名詞'; infl='固有名詞（Np）'; stem='—'
    elif core.startswith('N'): core_pos='名詞'; infl='名詞形（'+core+'）'; stem='—'
    elif core.startswith('V'):
        core_pos='動詞'; stem=STEM.get(core[1:2],'—'); infl=CONJ.get(core[2:3],'動詞形')+'（'+core+'）'
    elif core.startswith('A'): core_pos='形容詞・数詞'; stem='—'; infl='形容詞・数詞形（'+core+'）'
    elif core.startswith('R'): core_pos='前置詞'; stem='—'; infl='前置詞（'+core+'）'
    elif core.startswith('C'): core_pos='接続詞'; stem='—'; infl='接続詞（'+core+'）'
    elif core.startswith('P'): core_pos='代名詞'; stem='—'; infl='代名詞（'+core+'）'
    elif core.startswith('T'): core_pos='小辞'; stem='—'; infl='小辞（'+core+'）'
    elif core.startswith('D'): core_pos='副詞'; stem='—'; infl='副詞（'+core+'）'
    else: core_pos='機能語'; stem='—'; infl=core or '—'
    pos='＋'.join(prefixes+[core_pos]) if prefixes else core_pos
