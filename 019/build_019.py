...
def details(c,v):
    ...
    return [('本文の骨格',f'{c}:{v}は、{THEME[ref]}節です。反復・対照・場面転換がこの節固有の役割を形づくります。'),('文法',f'主要語 {w}（レンマ {l}）は、品詞 {p}、語幹 {s}、活用 {i}。語順と接続要素が発話または物語の進行を支えます。'),('ケティーブ／ケレー・本文伝承','狭義のケティーブ／ケレーは確認されません。古代訳・写本間の異読はK/Qと区別します。'),('ラビ・中世',RAB[ref]),('教父文学',PAT[ref]),('文献層と物語',LIT[ref]),('デボーショナルな受けとめ',DEV[ref])]
